...
class Device extends EventEmitter {
  ...
  purgeConnection(clientConnection) {
    ...
    Object.keys(this.transports).forEach((localId) => {
      const transport = this.transports[localId];
      const { subscriptions } = transport;
      transport.subscriptions = subscriptions.filter((subscription) => subscription.clientConnection != clientConnection);
    });
  }
  ...
  startApplication(appId) {
    ...
    switch (appId) {
      case androidMirroringAudioVideoAppId:
        this.startMirroringSession(appId, 'Android Mirroring');
        break;
      case castMirroringAudioVideoAppId:
  ...
  emitChallenge(challenge, respond) {
    this.emit('challenge', {
      challenge,
      respond
    });
  ...
  emitOffer(offer, sessionId, respond) {
    this.emit('offer', {
      offer,
      respond,
      sessionId
